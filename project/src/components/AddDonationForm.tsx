...
const AddDonationForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setLoadingMessage("Submitting donation...");
    try {
      const now = new Date();
      ...
      <div className="space-y-6 bg-white p-6 rounded-xl shadow-md border">
        <div className="flex flex-col items-center">
          ...
          <input
            ...
            className="mb-3"
          />
          {loading && <p className="mb-2 text-gray-500 font-sm">Uploading...</p>}
          {imageUrl && (
            <img
              src={imageUrl}
              alt="Uploaded"
          ...
          {loadingMessage && (<p className="mb-2 text-gray-500 font-sm">{loadingMessage}</p>)}
          {isImageSelected && (
