...
const Homepage = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        if (user) {
          if (user.role === "donate") navigate("/donor-dashboard");
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const monthlyData = Number(response.data.data.monthly_data);
        const impactScore = Number(response.data.data.impact_score);
        console.log(dailyData);
        animateCounter(setDailyMeals, dailyData);
        animateCounter(setMonthlyMeals, monthlyData);
        animateCounter(setImpactScore, impactScore);
        console.log(response.data);
      } catch (err) {
        console.log(err);
      }
  ...
  const handleCTAClick = (type: 'donate' | 'request' | 'login') => {
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="flex flex-row sm:flex-row gap-0 justify-center">
              <motion.button
                onClick={() => handleCTAClick('donate')}
                className="bg-orange-500/60 hover:bg-orange-500/80 text-white px-8 py-4 rounded-l-full font-semibold text-lg flex items-center justify-center space-x-2 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-orange-300/40"
              ...
              <motion.button
                ...
                className="bg-emerald-600/60 hover:bg-emerald-700/80 backdrop-blur-sm text-white px-8 py-4 rounded-r-full font-semibold text-lg flex items-center justify-center space-x-2 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-emerald-300/40"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
            ...
            <div className="flex flex-row sm:flex-row m-2 gap-2 justify-center">
              <motion.button
                onClick={() => handleCTAClick('login')}
                className="bg-indigo-500/60 hover:bg-indigo-600/80 backdrop-blur-sm text-white px-28 py-4 rounded-full font-semibold text-lg flex items-center justify-center space-x-2 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-indigo-300/40"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="text-emerald-100 text-lg font-bold group-hover:text-orange-200 transition-colors duration-300 uppercase tracking-wide">Meals Distributed This Month</div>
            </motion.div>
            ...
            <div className="flex justify-center space-x-2 mt-8">
              {successStories.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-colors duration-300 ${index === currentStory ? 'bg-emerald-600' : 'bg-gray-300'
                    }`}
                />
              ))}
            </div>
