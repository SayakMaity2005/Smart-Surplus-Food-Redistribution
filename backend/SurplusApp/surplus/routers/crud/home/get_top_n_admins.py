...
class TopAdminData:
    def __init__(self, name, impact_score, items_provided_in_month):
        self.name: str = name
...
async def get_top_n_admins(request: RequestSchema, db: Session = Depends(get_db)):
    ...
    top_n_items_provided_monthly_data = db.query(models.MonthlyData).order_by(
    ...
    for monthly_data in top_n_items_provided_monthly_data:
        top_n_admins.append(TopAdminData(monthly_data.admin.name, monthly_data.impact_score, monthly_data.items_provided))
