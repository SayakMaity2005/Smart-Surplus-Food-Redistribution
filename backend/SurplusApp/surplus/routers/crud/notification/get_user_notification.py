...
@router.get("/user/get-user-notification/")
async def get_user_notification(request_cookie: Request, db: Session = Depends(get_db)):
    ...
    user_notifications = db.query(models.UserNotification).filter(models.UserNotification.user_id == current_user.id).all()
    return {"message": "User notifications accessed successfully", "data": user_notifications, "isUnseen": isUnseen}